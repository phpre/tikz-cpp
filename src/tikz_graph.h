...
struct vertex {
    ...
    static constexpr vertex unselected_vertex( color p_color = COLOR_TEXT, double p_radius = 1.0,
                                               double  p_margin = .75,
  ...
  public:
    ...
    inline std::string label_for_pos( s32 p_posX, s32 p_posY ) const {
        if( m_labelPrefix != EMPTY_STR ) {
            return m_labelPrefix + "_" + std::to_string( p_posX ) + "_" + std::to_string( p_posY );
        } else {
    ...
    void print_vertex_on_coordinates( FILE* p_out, const vertex& p_vertex, s32 p_posX = 0,
                                      s32 p_posY = 0, const std::string& p_extraOptions = EMPTY_STR,
                                      u32 p_startIndent = 1, u32 p_indent = 4 ) const;
