...
namespace TIKZ {
    ...
    void vertex_grid::print_coordinates( FILE* p_out, u64 p_sizeX, u64 p_sizeY, s32 p_startX,
                                         s32 p_startY, const std::string& p_extraOptions,
                                         u64 p_startIndent, u64 p_indent ) const {
    ...
    void vertex_grid::print_vertices_on_coordinates( FILE* p_out, const vertex& p_vertex,
    ...
    void vertex_grid::print_vertex_on_coordinates( FILE* p_out, const vertex& p_vertex, s32 p_posX,
                                                   ...
                                                   u64 p_startIndent, u64 p_indent ) const {
        print_vertex( p_out, this->label_for_pos( p_posX, p_posY ), p_vertex, EMPTY_STR,
                      p_extraOptions, p_startIndent, p_indent );
    }

    void vertex_grid::print_diagonal_on_coordinates( FILE* p_out, const vertex& p_vertex,
                                                     u64 p_length, s32 p_startX, s32 p_startY,
                                                     const std::string& p_extraOptions,
                                                     u64 p_startIndent, u64 p_indent ) const {
        for( u64 i = 0; i < p_length; ++i ) {
            print_vertex( p_out, this->label_for_pos( p_startX + i, p_startY + i ), p_vertex,
                          EMPTY_STR, p_extraOptions, p_startIndent, p_indent );
        }
    }

    void vertex_grid::print_new_vertex( FILE* p_out, const vertex& p_vertex, s32 p_posX, s32 p_posY,
                                        const std::string& p_extraOptions, u64 p_startIndent,
                                        u64 p_indent ) const {
        print_vertex( p_out, this->point_for_pos( p_posX, p_posY ), p_vertex,
                      this->label_for_pos( p_posX, p_posY ), p_extraOptions, p_startIndent,
                      p_indent );
    }

    void vertex_grid::print_vertices( FILE* p_out, const vertex& p_vertex, u64 p_sizeX, u64 p_sizeY,
    ...
    void vertex_grid::print_diagonal( FILE* p_out, const vertex& p_vertex, u64 p_length,
                                      s32 p_startX, s32 p_startY, const std::string& p_extraOptions,
                                      u64 p_startIndent, u64 p_indent ) const {
        for( u64 i = 0; i < p_length; ++i ) {
    ...
    void print_vertex( FILE* p_out, const std::string& p_position, const vertex& p_vertex,
                       const std::string& p_name, const std::string& p_extraOptions,
    ...
    void print_selected_arrow( FILE* p_out, const std::string& p_startPos,
                               ...
                               double p_angle, u64 p_startIndent, u64 p_indent ) {
    ...
    void print_selected_arrow( FILE* p_out, tikz_point p_startPos, const tikz_point p_endPos,
        ...
        print_selected_arrow( p_out, buffer1, buffer2, p_lineColor, p_fillColor, p_angle,
                              p_startIndent, p_indent );
    }

    void print_arrow( FILE* p_out, const std::string& p_startPos, const std::string& p_endPos,
                      const std::string& p_lineWidth, color p_color, u64 p_startIndent,
                      u64 p_indent ) {
