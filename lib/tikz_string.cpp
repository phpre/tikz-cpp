...
namespace TIKZ {
        ...
        for( u32 x = p_fragT.closed_begin( ); x <= p_fragT.open_end( ); ++x ) {
            for( u32 y = p_fragP.closed_begin( ); y <= p_fragP.open_end( ); ++y ) {
                ...
                if( x > p_fragT.closed_begin( ) && y > p_fragP.closed_begin( ) ) {
                    if( p_P[ y - 1 ] == p_T[ x - 1 ] ) {
                        print_arrow( p_out, vg.label_for_pos( x - 1, y - 1 ),
                                     vg.label_for_pos( x, y ), LW_THICK_SUPPORT_LINE,
                                     MAT_COL.deemphasize( ) );
                    } else {
                        print_arrow( p_out, vg.label_for_pos( x - 1, y - 1 ),
                                     vg.label_for_pos( x, y ), LW_SUPPORT_LINE,
                                     SUB_COL.deemphasize_strong( ) );
                    }
                }
            }
        }
    ...
    void print_graph_slice_on_coordinates( FILE* p_out, const vertex_grid& p_vg,
        ...
        if( p_topColor.is_non_empty( ) ) {
            ...
            for( auto i = x; i <= xm; ++i ) {
                p_vg.print_vertex_on_coordinates( p_out, portal, i, y );
            }
        }
            ...
            if( p_guideLines ) {
                ...
                if( fP.closed_begin( ) && i + 0LU != frag.closed_end( ) ) {
                    posP = tikz_point{ fragT.open_end( ) * CHAR_WIDTH
                                       ...
                                       fP.closed_begin( ) * -CHAR_HEIGHT };
                    print_connection( p_out,
                                      vg.point_for_pos( fT.open_end( ), fP.closed_begin( ) )
                                          + tikz_point{ .2, 0.0 },
                                      posP + tikz_point{ -.2, 0.0 }, LINE_CONN,
                                      LW_SUPPORT_LINE + ", "
                                          + ( i % 5 ? COLOR_TEXT.deemphasize_strong( )
                ...
                if( i && fP.open_end( ) != frag.open_end( ) ) {
                    ...
                    print_connection( p_out,
                                      vg.point_for_pos( fT.closed_begin( ), fP.open_end( ) )
                                          + tikz_point{ -.2, 0.0 },
                                      posP + tikz_point{ .2, 0.0 }, LINE_CONN,
                                      LW_SUPPORT_LINE + ", "
                                          + ( i % 5 ? COLOR_TEXT.deemphasize_strong( )
